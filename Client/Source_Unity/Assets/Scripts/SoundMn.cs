...
public class SoundMn
{
	...
	public void getStrOption()
	{
		...
		if (Main.isPC)
		{
			...
			{
				(!Char.isPaintAura) ? (str2 + mResources.aura_off.Trim()) : (str + mResources.aura_off.Trim()),
				(!Char.isPaintAura2) ? (str2 + mResources.aura_off_2.Trim()) : (str + mResources.aura_off_2.Trim()),
				(!GameCanvas.isPlaySound) ? (str2 + mResources.turnOffSound.Trim()) : (str + mResources.turnOffSound.Trim()),
		...
		Panel.strCauhinh = new string[5]
		{
			...
			(!Char.isPaintAura2) ? (str2 + mResources.aura_off_2.Trim()) : (str + mResources.aura_off_2.Trim()),
			(!GameCanvas.isPlaySound) ? (str2 + mResources.turnOffSound.Trim()) : (str + mResources.turnOffSound.Trim()),
			(!GameCanvas.lowGraphic) ? (str2 + mResources.cauhinhthap.Trim()) : (str + mResources.cauhinhthap.Trim()),
			text
		};
	}
