		...
		try
		{
			...
			case 65:
			{
				...
				if (ItemTime.isExistMessage(b36))
				{
					if (num91 != 0)
					{
						ItemTime.getMessageById(b36).initTimeText(b36, text3, num91);
					}
					else
					{
				...
				else
				{
					...
					itemTime.initTimeText(b36, text3, num91);
			...
			case 112:
			{
				...
				else if (b43 == 1)
				{
					...
					for (int num109 = 0; num109 < b44; num109++)
					{
						GameCanvas.panel.speacialTabName[num109] = new string[2];
						string[] array8 = Res.split(msg.reader().readUTF(), "\n", 0);
						if (array8.Length == 2)
						{
							GameCanvas.panel.speacialTabName[num109] = array8;
						}
						if (array8.Length == 1)
						{
							GameCanvas.panel.speacialTabName[num109][0] = array8[0];
							GameCanvas.panel.speacialTabName[num109][1] = string.Empty;
						}
						int num110 = msg.reader().readByte();
						Char.myCharz().infoSpeacialSkill[num109] = new string[num110];
						Char.myCharz().imgSpeacialSkill[num109] = new short[num110];
						for (int num111 = 0; num111 < num110; num111++)
						{
							Char.myCharz().imgSpeacialSkill[num109][num111] = msg.reader().readShort();
							Char.myCharz().infoSpeacialSkill[num109][num111] = msg.reader().readUTF();
						}
					}
			...
			case -96:
			{
				...
				for (int num149 = 0; num149 < b64; num149++)
				{
			...
			case -95:
			{
				...
				if (b37 == 0)
				{
					...
					int num93 = msg.readInt3Byte();
					SoundMn.gI().explode_1();
					if (num92 == Char.myCharz().charID)
					{
						Char.myCharz().mobMe = new Mob(num92, isDisable: false, isDontMove: false, isFire: false, isIce: false, isWind: false, templateId, 1, num93, 0, num93, (short)(Char.myCharz().cx + ((Char.myCharz().cdir != 1) ? (-40) : 40)), (short)Char.myCharz().cy, 4, 0);
					...
					else
					{
						...
						if (@char != null)
						{
							Mob mob5 = new Mob(num92, isDisable: false, isDontMove: false, isFire: false, isIce: false, isWind: false, templateId, 1, num93, 0, num93, (short)@char.cx, (short)@char.cy, 4, 0);
							mob5.isMobMe = true;
							@char.mobMe = mob5;
						...
						{
							Mob mob6 = GameScr.findMobInMap(num92);
							if (mob6 == null)
							{
								mob6 = new Mob(num92, isDisable: false, isDontMove: false, isFire: false, isIce: false, isWind: false, templateId, 1, num93, 0, num93, -100, -100, 4, 0);
								mob6.isMobMe = true;
				...
				if (b37 == 2)
				{
					...
					if (num95 == Char.myCharz().charID)
					{
						...
						if (@char != null)
						{
							...
							{
								@char.doInjure(num97, 0, isCrit: false, isMob: true);
							}
							else
							{
								Char.myCharz().mobMe.dame = num97;
					...
					{
						mob = GameScr.findMobInMap(num95);
						if (mob != null)
						{
							if (num96 == Char.myCharz().charID)
							{
								...
								{
									Char.myCharz().doInjure(num97, 0, isCrit: false, isMob: true);
								}
								else
								{
									mob.dame = num97;
				...
				if (b37 == 3)
				{
					...
					@char = null;
					@char = ((Char.myCharz().charID != num98) ? GameScr.findCharInMap(num98) : Char.myCharz());
					if (@char != null)
					{
						...
						if (mob != null)
						{
							...
							{
								GameScr.startFlyText("-" + num99, mob.x, mob.y - mob.h, 0, -2, mFont.ORANGE);
							}
						}
					}
				}
				...
				if (b37 == 5)
				{
					...
					int num101 = msg.readInt3Byte();
					int hp2 = msg.readInt3Byte();
					@char = null;
					@char = ((num100 != Char.myCharz().charID) ? GameScr.findCharInMap(num100) : Char.myCharz());
					if (@char == null)
					{
						return;
					}
					...
					{
						GameScr.startFlyText("-" + num101, mob7.x, mob7.y - mob7.h, 0, -2, mFont.ORANGE);
					}
				}
				if (b37 == 6)
				{
				...
				if (b37 == 7)
				{
					...
					if (num103 == Char.myCharz().charID)
					{
						...
						{
							if (((Mob)GameScr.vMob.elementAt(num104)).mobId == num103)
							{
					...
					else
					{
						...
						for (int num105 = 0; num105 < GameScr.vMob.size(); num105++)
						{
							if (((Mob)GameScr.vMob.elementAt(num105)).mobId == num103)
							{
								GameScr.vMob.removeElementAt(num105);
							}
						}
						if (@char != null)
						{
			...
			case -91:
			{
				...
				for (int num143 = 0; num143 < b61; num143++)
				{
					GameCanvas.panel.mapNames[num143] = msg.reader().readUTF();
					GameCanvas.panel.planetNames[num143] = msg.reader().readUTF();
				}
			...
			case -90:
			{
				...
				@char = ((Char.myCharz().charID != num87) ? GameScr.findCharInMap(num87) : Char.myCharz());
				if (b34 != -1)
				{
					...
					Res.err("====> Cmd: -90 UPDATE_BODY   \n  isMonkey= " + b35 + " head=  " + num88 + " body= " + num89 + " legU= " + num90);
					if (@char != null)
					{
						if (@char.charID == num87)
						{
							@char.isMask = true;
							@char.isMonkey = b35;
							if (@char.isMonkey != 0)
			...
			case -85:
			{
				...
				if (b54 == 0)
				{
					int num132 = msg.reader().readUnsignedShort();
					Res.outz("lent =" + num132);
					sbyte[] data3 = new sbyte[num132];
					msg.reader().read(ref data3, 0, num132);
					GameScr.imgCapcha = Image.createImage(data3, 0, num132);
					GameScr.gI().keyInput = "-----";
			...
			case -76:
			{
				...
				if (b49 == 0)
				{
					...
					for (int num123 = 0; num123 < b50; num123++)
					{
						Char.myCharz().arrArchive[num123] = new Archivement();
						Char.myCharz().arrArchive[num123].info1 = num123 + 1 + ". " + msg.reader().readUTF();
						Char.myCharz().arrArchive[num123].info2 = msg.reader().readUTF();
						Char.myCharz().arrArchive[num123].money = msg.reader().readShort();
						Char.myCharz().arrArchive[num123].isFinish = msg.reader().readBoolean();
						Char.myCharz().arrArchive[num123].isRecieve = msg.reader().readBoolean();
					}
				...
				else if (b49 == 1)
				{
					...
					if (Char.myCharz().arrArchive[num124] != null)
					{
						Char.myCharz().arrArchive[num124].isRecieve = true;
					}
				}
				break;
			}
			case -74:
			{
				...
				if (b69 == 0)
				{
					...
					int num172 = (text7 == null || !(text7 != string.Empty)) ? (-1) : int.Parse(text7);
					if (num172 == -1 || num172 != num171)
					{
				...
				if (b69 == 1)
				{
					...
					short num173 = (short)(ServerListScreen.nBig = msg.reader().readShort());
				...
				if (b69 == 3 && flag10)
				{
					...
					Res.outz("last version= " + num175);
					Rms.saveRMSString("ResVersion", num175 + string.Empty);
					Service.gI().getResource(3, null);
					GameCanvas.endDlg();
					SplashScr.imgLogo = null;
					SmallImage.loadBigRMS();
			...
			case -62:
			{
				...
				if (b65 > 0)
				{
					ClanImage clanImage3 = ClanImage.getClanImage((short)num150);
					if (clanImage3 != null)
					{
						clanImage3.idImage = new short[b65];
						for (int num151 = 0; num151 < b65; num151++)
						{
							clanImage3.idImage[num151] = msg.reader().readShort();
							if (clanImage3.idImage[num151] > 0)
							{
								SmallImage.vKeys.addElement(clanImage3.idImage[num151] + string.Empty);
							}
						}
					}
				}
			...
			case -64:
			{
				...
				{
					for (int num159 = 0; num159 < 54; num159++)
					{
						@char.removeEffChar(0, 201 + num159);
					}
				}
				Res.outz("cmd:-64 UPDATE BAG PLAER = " + ((@char != null) ? @char.cName : string.Empty) + num157 + " BAG ID= " + num158);
				break;
			}
			case -63:
			{
				Res.outz("GET BAG");
				int num128 = msg.reader().readUnsignedByte();
				sbyte b51 = msg.reader().readByte();
				ClanImage clanImage2 = new ClanImage();
				clanImage2.ID = num128;
				if (b51 > 0)
				{
					clanImage2.idImage = new short[b51];
					for (int num129 = 0; num129 < b51; num129++)
					{
						clanImage2.idImage[num129] = msg.reader().readShort();
						Res.outz("ID=  " + num128 + " frame= " + clanImage2.idImage[num129]);
					}
					ClanImage.idImages.put(num128 + string.Empty, clanImage2);
				}
				break;
			}
			case -57:
			...
			case -52:
			{
				...
				if (b66 == 2)
				{
					...
					for (int num152 = 0; num152 < GameCanvas.panel.myMember.size(); num152++)
					{
						Member member4 = (Member)GameCanvas.panel.myMember.elementAt(num152);
						if (member4.ID == member3.ID)
						{
							...
							GameCanvas.panel.myMember.insertElementAt(o2, num152);
							return;
						}
					}
				}
			...
			case -50:
			{
				...
				for (int num156 = 0; num156 < b67; num156++)
				{
			...
			case -47:
			{
				...
				else
				{
					...
					for (int num130 = 0; num130 < GameCanvas.panel.clans.Length; num130++)
					{
						GameCanvas.panel.clans[num130] = new Clan();
						GameCanvas.panel.clans[num130].ID = msg.reader().readInt();
						GameCanvas.panel.clans[num130].name = msg.reader().readUTF();
						GameCanvas.panel.clans[num130].slogan = msg.reader().readUTF();
						GameCanvas.panel.clans[num130].imgID = msg.reader().readUnsignedByte();
						GameCanvas.panel.clans[num130].powerPoint = msg.reader().readUTF();
						GameCanvas.panel.clans[num130].leaderName = msg.reader().readUTF();
						GameCanvas.panel.clans[num130].currMember = msg.reader().readUnsignedByte();
						GameCanvas.panel.clans[num130].maxMember = msg.reader().readUnsignedByte();
						GameCanvas.panel.clans[num130].date = msg.reader().readInt();
					}
				}
			...
			case -61:
			{
				...
				{
					if (GameScr.findCharInMap(num86) != null)
					{
						GameScr.findCharInMap(num86).clanID = msg.reader().readInt();
						if (GameScr.findCharInMap(num86).clanID == -2)
						{
							GameScr.findCharInMap(num86).isCopy = true;
						}
					}
				}
			...
			case -37:
			{
				...
				if (b68 == 0)
				{
					...
					Char.myCharz().arrItemBody = new Item[num164];
					for (int num165 = 0; num165 < num164; num165++)
					{
						short num166 = msg.reader().readShort();
						if (num166 != -1)
						{
							Char.myCharz().arrItemBody[num165] = new Item();
							Char.myCharz().arrItemBody[num165].template = ItemTemplates.get(num166);
							int num167 = Char.myCharz().arrItemBody[num165].template.type;
							Char.myCharz().arrItemBody[num165].quantity = msg.reader().readInt();
							Char.myCharz().arrItemBody[num165].info = msg.reader().readUTF();
							Char.myCharz().arrItemBody[num165].content = msg.reader().readUTF();
							int num168 = msg.reader().readUnsignedByte();
							if (num168 != 0)
							{
								Char.myCharz().arrItemBody[num165].itemOption = new ItemOption[num168];
								for (int num169 = 0; num169 < Char.myCharz().arrItemBody[num165].itemOption.Length; num169++)
								{
									int num170 = msg.reader().readUnsignedByte();
									int param6 = msg.reader().readUnsignedShort();
									if (num170 != -1)
									{
										Char.myCharz().arrItemBody[num165].itemOption[num169] = new ItemOption(num170, param6);
									}
								}
							}
							switch (num167)
							{
							case 0:
								Char.myCharz().body = Char.myCharz().arrItemBody[num165].template.part;
								break;
							case 1:
								Char.myCharz().leg = Char.myCharz().arrItemBody[num165].template.part;
								break;
							}
						}
					}
			...
			case -35:
			{
				...
				if (b45 == 0)
				{
					...
					Char.myCharz().arrItemBox = new Item[num112];
					...
					for (int num113 = 0; num113 < num112; num113++)
					{
						...
						if (num114 != -1)
						{
							Char.myCharz().arrItemBox[num113] = new Item();
							Char.myCharz().arrItemBox[num113].template = ItemTemplates.get(num114);
							Char.myCharz().arrItemBox[num113].quantity = msg.reader().readInt();
							Char.myCharz().arrItemBox[num113].info = msg.reader().readUTF();
							Char.myCharz().arrItemBox[num113].content = msg.reader().readUTF();
							int num115 = msg.reader().readUnsignedByte();
							if (num115 != 0)
							{
								Char.myCharz().arrItemBox[num113].itemOption = new ItemOption[num115];
								for (int num116 = 0; num116 < Char.myCharz().arrItemBox[num113].itemOption.Length; num116++)
								{
									int num117 = msg.reader().readUnsignedByte();
									int param5 = msg.reader().readUnsignedShort();
									if (num117 != -1)
									{
										Char.myCharz().arrItemBox[num113].itemOption[num116] = new ItemOption(num117, param5);
									}
								}
							}
			...
			case -45:
			{
				...
				Res.outz("skill type= " + b55 + "   player use= " + num138);
				if (b55 == 0)
				{
					Res.outz("id use= " + num138);
					if (Char.myCharz().charID != num138)
					{
						@char = GameScr.findCharInMap(num138);
						if ((TileMap.tileTypeAtPixel(@char.cx, @char.cy) & 2) == 2)
						{
							@char.setSkillPaint(GameScr.sks[num139], 0);
						}
						else
						{
							@char.setSkillPaint(GameScr.sks[num139], 1);
							@char.delayFall = 20;
						}
					}
					else
				...
				if (b55 == 1 && num138 != Char.myCharz().charID)
				{
					GameScr.findCharInMap(num138).isCharge = true;
				}
				if (b55 == 3)
				{
					if (num138 == Char.myCharz().charID)
					{
					...
					{
						GameScr.findCharInMap(num138).isCharge = false;
					}
				}
				if (b55 == 4)
				{
					...
					else if (GameScr.findCharInMap(num138) != null)
					{
						switch (GameScr.findCharInMap(num138).cgender)
						{
						case 0:
							GameScr.findCharInMap(num138).useChargeSkill(isGround: false);
							break;
						case 1:
							GameScr.findCharInMap(num138).useChargeSkill(isGround: true);
							break;
						}
						GameScr.findCharInMap(num138).skillTemplateId = num139;
						GameScr.findCharInMap(num138).isUseSkillAfterCharge = true;
						GameScr.findCharInMap(num138).seconds = msg.reader().readShort();
						GameScr.findCharInMap(num138).last = mSystem.currentTimeMillis();
					}
				}
				if (b55 == 5)
				{
					...
					{
						GameScr.findCharInMap(num138).stopUseChargeSkill();
					}
				}
				if (b55 == 6)
				{
					if (num138 == Char.myCharz().charID)
					{
						Char.myCharz().setAutoSkillPaint(GameScr.sks[num139], 0);
					}
					else if (GameScr.findCharInMap(num138) != null)
					{
						GameScr.findCharInMap(num138).setAutoSkillPaint(GameScr.sks[num139], 0);
						SoundMn.gI().gong();
					}
				}
				if (b55 == 7)
				...
				if (b55 == 8 && num138 != Char.myCharz().charID && GameScr.findCharInMap(num138) != null)
				{
					GameScr.findCharInMap(num138).setAutoSkillPaint(GameScr.sks[num139], 0);
				}
				break;
			...
			case -67:
			{
				...
				try
				{
					...
					SmallImage.imgNew[num163].img = createImage(array16);
				}
				catch (Exception)
				{
					array16 = null;
				...
				{
					Rms.saveRMS(mGraphics.zoomLevel + "Small" + num163, array16);
				}
				break;
			}
			case -66:
			{
				...
				EffectData effDataById = Effect.getEffDataById(num145);
				...
				if (num145 == 78)
				{
					...
					for (int num146 = 0; num146 < b63; num146++)
					{
						int num147 = msg.reader().readUnsignedByte();
						array15[num146] = new short[num147];
						for (int num148 = 0; num148 < num147; num148++)
						{
							array15[num146][num148] = msg.reader().readShort();
						}
					}
			...
			case -32:
			{
				...
				try
				{
					array12 = new sbyte[num136];
					for (int num137 = 0; num137 < num136; num137++)
					{
						array12[num137] = msg.reader().readByte();
					}
					image = Image.createImage(array12, 0, num136);
				...
				if (array12 != null)
				{
					...
					{
						Rms.saveRMS(mGraphics.zoomLevel + "bgItem" + num135, array12);
					}
			...
			case 58:
			{
				...
				char10.moveFast[1] = num161;
				char10.moveFast[2] = num162;
				try
				{
			...
			case 88:
			{
				...
				GameCanvas.inputDlg.show(info4, new Command(mResources.ACCEPT, GameCanvas.instance, 88818, num160), TField.INPUT_TYPE_ANY);
				break;
			}
			case 27:
			{
			...
			case 33:
				...
				for (int num144 = 0; num144 < Char.myCharz().npcFocus.template.menu.Length; num144++)
				{
					string[] array13 = Char.myCharz().npcFocus.template.menu[num144];
			...
			case 40:
			{
				...
				try
				{
					...
					for (int num134 = 0; num134 < array9.Length; num134++)
					{
						array11[num134] = msg.reader().readShort();
					}
				}
			...
			case 50:
			{
				...
				for (int num131 = 0; num131 < b53; num131++)
				{
			...
			case 43:
				...
				try
				{
					...
					Char.myCharz().x_hint = num125;
					Char.myCharz().y_hint = num126;
					Res.outz("CMD   TASK_UPDATE:43_mapID =    x|y " + num125 + "|" + num126);
					for (int num127 = 0; num127 < TileMap.vGo.size(); num127++)
					{
						Res.outz("===> " + TileMap.vGo.elementAt(num127));
					}
				}
				catch (Exception)
				{
			...
			case -21:
			{
				...
				{
					if (((ItemMap)GameScr.vItemMap.elementAt(num122)).itemMapID == itemMapID)
					{
						GameScr.vItemMap.removeElementAt(num122);
						break;
					}
				}
				break;
			...
			case -20:
			{
				...
				{
					ItemMap itemMap2 = (ItemMap)GameScr.vItemMap.elementAt(num121);
			...
			case -19:
			{
				...
				{
					ItemMap itemMap = (ItemMap)GameScr.vItemMap.elementAt(num120);
			...
			case -18:
			{
				...
				int num119 = msg.reader().readByte();
				GameScr.vItemMap.addElement(new ItemMap(msg.reader().readShort(), Char.myCharz().arrItemBag[num119].template.id, Char.myCharz().cx, Char.myCharz().cy, msg.reader().readShort(), msg.reader().readShort()));
				Char.myCharz().arrItemBag[num119] = null;
				break;
			}
			case 68:
			{
				...
				ItemMap o = new ItemMap(num118, itemMapID, itemTemplateID, x, y, r);
			...
			case 32:
			{
				...
				try
				{
					short num83 = (short)(npc3.avatar = msg.reader().readShort());
				}
				catch (Exception)
				{
				}
			...
			case -2:
			{
				...
				GameScr.startFlyText((num193 <= 0) ? (string.Empty + num193) : ("+" + num193), Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch - 10, 0, -2, mFont.YELLOW);
				break;
			}
			case 95:
			{
			...
			case 97:
			{
				...
				for (int num180 = 0; num180 < Char.myCharz().taskOrders.size(); num180++)
				{
					TaskOrder taskOrder = (TaskOrder)Char.myCharz().taskOrders.elementAt(num180);
			...
			case -1:
			{
				...
				Char.myCharz().xu += num192;
				Char.myCharz().xuStr = mSystem.numberTostring(Char.myCharz().xu);
				Char.myCharz().yen -= num192;
				GameScr.startFlyText("+" + num192, Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch - 10, 0, -2, mFont.YELLOW);
				break;
			}
			case -3:
			{
				...
				if (b78 == 0)
				{
					Char.myCharz().cPower += num198;
				}
				if (b78 == 1)
				{
					Char.myCharz().cTiemNang += num198;
				}
				if (b78 == 2)
				{
					Char.myCharz().cPower += num198;
				...
				{
					GameScr.startFlyText(((num198 <= 0) ? string.Empty : "+") + num198, Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch, 0, -4, mFont.GREEN);
					if (num198 > 0 && Char.myCharz().petFollow != null && Char.myCharz().petFollow.smallID == 5002)
					{
						ServerEffect.addServerEffect(55, Char.myCharz().petFollow.cmx, Char.myCharz().petFollow.cmy, 1);
						ServerEffect.addServerEffect(55, Char.myCharz().cx, Char.myCharz().cy, 1);
					}
			...
			case -73:
			{
				...
				for (int num191 = 0; num191 < GameScr.vNpc.size(); num191++)
				{
					Npc npc7 = (Npc)GameScr.vNpc.elementAt(num191);
			...
			case -5:
			{
				...
				try
				{
					...
					else
					{
						...
						{
							char14.removeEffChar(0, 201 + num188);
						}
					}
				}
			...
			case -7:
			{
				...
				for (int num182 = 0; num182 < GameScr.vCharInMap.size(); num182++)
				{
					...
					try
					{
						char13 = (Char)GameScr.vCharInMap.elementAt(num182);
					}
			...
			case -6:
			{
				...
				int num181 = msg.reader().readInt();
				for (int num197 = 0; num197 < GameScr.vCharInMap.size(); num197++)
				{
					Char char15 = (Char)GameScr.vCharInMap.elementAt(num197);
					if (char15 != null && char15.charID == num181)
					{
						...
						{
							GameScr.vCharInMap.removeElementAt(num197);
						}
						return;
					}
				}
			...
			case -9:
			{
				...
				if (mob9 != null)
				{
					...
					if (flag11)
					{
						GameScr.startFlyText("-" + num184, mob9.x, mob9.getY() - mob9.getH(), 0, -2, mFont.FATAL);
					}
					else if (num184 == 0)
					{
						mob9.x = mob9.xFirst;
					...
					{
						GameScr.startFlyText("-" + num184, mob9.x, mob9.getY() - mob9.getH(), 0, -2, mFont.ORANGE);
					}
				}
			...
			case -12:
			{
				...
				if (mob9 != null && mob9.status != 0 && mob9.status != 0)
				{
					...
					try
					{
						...
						for (int num195 = 0; num195 < b77; num195++)
						{
			...
			case 18:
			{
				...
				for (int num176 = 0; num176 < b70; num176++)
				{
					int charId = msg.reader().readInt();
